...
# Adapted from fla/ops/kda/chunk.py for NPU (Triton-only, no CP support).
...
def chunk_kda(
    ...
    **kwargs,
    ...
    r"""
    KDA chunk attention (NPU Triton-only version, no Context Parallel support).
    ...
    Args:
        ...
        transpose_state_layout (bool):
            Whether hidden state h uses transposed layout [B, NT, H, V, K]. Default: `False`.
    ...
    Returns:
        ...
        - Inference mode (return_intermediate_states=True): A tuple (o, final_state, h)
            ...
            final_state (torch.Tensor):
                Final state of shape `[N, H, K, V]` if `output_final_state=True` else `None`.
            h (torch.Tensor):
                Intermediate states of shape `[B, NT, H, K, V]` and dtype `bfloat16`.
    """
