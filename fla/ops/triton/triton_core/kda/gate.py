...
from ..utils import IS_AMD, autocast_custom_bwd, autocast_custom_fwd, autotune_cache_kwargs, check_shared_mem, filter_safe_configs, input_guard, _get_autotune_layer
...
def kda_gate_fwd(
    ...
    # NPU coreDim limit: filter autotune configs where grid product > MAX_COREDIM
    _fwd_autotune = _get_autotune_layer(kda_gate_fwd_kernel)
    _orig_fwd_cfg = _fwd_autotune.configs
    _safe_fwd_cfg = filter_safe_configs(kda_gate_fwd_kernel, grid)
    _fwd_autotune.configs = _safe_fwd_cfg
    ...
    try:
        ...
        _fwd_autotune.configs = _orig_fwd_cfg
    return yg
...
def kda_gate_chunk_cumsum(
    ...
    **kwargs,
    ...
    # NPU coreDim limit: filter autotune configs where grid product > MAX_COREDIM
    _cumsum_autotune = _get_autotune_layer(kda_gate_chunk_cumsum_vector_kernel)
    _orig_cumsum_cfg = _cumsum_autotune.configs
    _safe_cumsum_cfg = filter_safe_configs(kda_gate_chunk_cumsum_vector_kernel, grid, runtime_meta={'S': S})
    _cumsum_autotune.configs = _safe_cumsum_cfg
    ...
    try:
        ...
        _cumsum_autotune.configs = _orig_cumsum_cfg
    return g